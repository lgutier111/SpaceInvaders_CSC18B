    ...
    private void moveEnemyEntities(double delta) {
        ...
        if (getEnemyEntities().size() > 0) {
            for (int i = 0; i < getEnemyEntities().size(); i++) {
                ...
                if (entity.getX() < tileMap.getX() - 50 || entity.getX() > tileMap.getX() + tileMap.getWidth() + 50 || entity.getY() > ResourceFactory.get().getGameWindow().getHeight() + 50 || entity.getY() < - 200) {
